module github.com/jou66jou/go-myresume

go 1.12

require (
	code.sajari.com/docconv v1.1.0
	github.com/JalfResi/justext v0.0.0-20170829062021-c0282dea7198 // indirect
	github.com/advancedlogic/GoOse v0.0.0-20191112112754-e742535969c1 // indirect
	github.com/golang/protobuf v1.3.2 // indirect
	github.com/gorilla/mux v1.7.3
	github.com/gorilla/websocket v1.4.1
	github.com/jung-kurt/gofpdf v1.16.2
	github.com/labstack/echo v3.3.10+incompatible
	github.com/labstack/gommon v0.3.0 // indirect
	github.com/levigross/exp-html v0.0.0-20120902181939-8df60c69a8f5 // indirect
	github.com/phpdave11/gopdf v0.9.0
	github.com/rakyll/statik v0.1.6
	github.com/sajari/docconv v1.1.0 // indirect
	github.com/signintech/gopdf v0.9.4 // indirect
	github.com/sirupsen/logrus v1.4.2
	github.com/unidoc/unipdf/v3 v3.3.0
	github.com/zserge/webview v0.0.0-20191103184548-1a9ebffc2601
	golang.org/x/crypto v0.0.0-20191202143827-86a70503ff7e // indirect
)
